...
const company = {
  popularCompany: async (uid) => {
    const query = `SELECT company_name,company_follow, company_industry, company_img, carousel_view_title FROM ${table};`;
  ...
  interestedCompany: async (uid) => {
    const query = `SELECT company_idx, company_img, company_hearts, company_name, company_location, recruit_position, carousel_view_title FROM ${table};`;
